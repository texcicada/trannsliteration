...
\texttt{lawcite} is built on the \texttt{ext-authortitle-ibid} style that comes with Biblatex. All the commands and drivers in that style are available for use.
...
\p For convenience, \texttt{lawcite} has a numbering mechanism using the \cmd{p} command for numbering (logical) paragraphs.
...
\p A pair of helper wrapper macros have also been defined: \cmd{paradef} can be used instead of \cmd{label} -- \texttt{\textbackslash paradef\{eg:b\}} is the same as typing \texttt{\textbackslash label\{eg:b\}} -- \paradef{eg:b}and \cmd{pararef} to do a ``para \cmd{ref}'' -- so that \texttt{\textbackslash pararef\{eg:b\}} is the same as typing \texttt{para \textbackslash ref\{eg:b\}}.

\p See \pararef{eg:b}. The reference paragraph format is sensitive to the lawcitestyle.

\setnumparshiftleft{\kern-2em}
\p Independently of the lawcitestyle, paragraph numbers are formatted via a dozen or so settings. For example, \cmd{setnumparshiftleft}\{\textbackslash kern-2em\} moves the paragraph number 2em to the left.
