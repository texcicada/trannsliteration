...
\bigskip{\renewcommand{\qqcitecasewidth}{0.68}
\qqcitefn{q10}\footnotetext{\lcinline[Quoted in][17]{atkinson}.}
}
...
This particular bibentry has its compulsory unique key \circnum{➀}, and contains the information to construct the case name \circnum{➁}, the medium neutral citation \circnum{➂}, one printed report \circnum{➃}, and some hyperlinking data.
...
@case\{araujo,	\circnum{➀}
			partya =  \{R\},	 \circnum{➁}
			partyb =  \{Araujo\},	 \circnum{➁}
			caseshortname =  \{Araujo\},	
			caseyear =  \{2000\},	 \circnum{➂}
			courtname =  \{SCC\},	 \circnum{➂}
			casenumber =  \{65\},	\circnum{➂}
			pagination =  \{atparagraph\},	\circnum{➂}
			mncurl =  \{https://decisions.scc-csc.ca/scc-csc/scc-csc/en/item/1830/index.do\},
			reportyear =  \{2000\},	 \circnum{➃}
			volyearneeded =  \{true\},  \circnum{➃}
			reportvolume =  \{2\},		\circnum{➃}
			reportseries =  \{SCR\}, \circnum{➃}	
			reportpage =  \{992\},	   \circnum{➃}
...
Compiling with latex, then biber, then latex again, will produce a footnote like this when McGill-style is active:
...
\subsection{Numbered Paragraphs}
{\parskip = 6pt%start group
\p For convenience, \texttt{lawcite} has a numbering mechanism using the \cmd{p} command for numbering (logical) paragraphs. To better visually mark the paragraphing, in this section the separation between paragraphs has been set to a non-zero amount (\texttt{\textbackslash parskip = 6pt}). Alternatively, the paragraph numbering could have been emboldened or otherwise re-styled.
...
\p A pair of helper wrapper macros have also been defined: one, \cmd{paradef}, can be used instead of \cmd{label} -- 
...
-- \paradef{eg:b}and the other, \cmd{pararef}, does a ``para \cmd{ref}'' -- so that 
...
\p See \pararef{eg:b}. The reference paragraph format is sensitive to the lawcitestyle.

\setnumparshiftleft{\kern-2em}
\p Independently of the lawcitestyle, paragraph numbers are formatted, and formattable, via a dozen or so settings. For example, \cmd{setnumparshiftleft}\{\textbackslash kern-2em\} moves the paragraph number 2em to the left.

\setnumparfillright{\hspace{3em}}
\p And \cmd{setnumparfillright}\{\textbackslash hspace\{3em\}\} inserts 3em of space after the parnum (or parnum delimiter, if any). And so on.
...
\subsection{Citation Commands - An Overiew}
...
Here, the \cmd{lawcite} citation command does most of the work in the legal space. There are other citation commands for various nuances, as well.

\bigskip
\textsc{Case}:
\examplebox{\bemph{\cmd{lawcite}\brackets{27}\braces{colchester}} $\mapsto$\par\smallskip\qcitetext{q13}\lawcite[27]{colchester}}

\textsc{Statute}:
\examplebox{\bemph{\cmd{lawcite}\braces{ppi}} $\mapsto$\par\smallskip\lawcite{ppi}}

\textsc{Law Journal article}:
\examplebox{\bemph{\cmd{ljfootcite}\brackets{\cmd{nopp} 625}\braces{ryder}} $\mapsto$\par\smallskip\qcitetext{q14}\ljfootcite[\nopp 625]{ryder}}

In the abstract, the field layout for a generic @case bibentry in the current lawcitestyle is shown in the following, with all citations using \cmd{lawcite} and the square brackets around \texttt{reportyear} being activated by the existence of the \texttt{volyearneeded} field. \texttt{partysep}, if not present and if required, defaults to \texttt{v}.

Illustration of @case fields: admiralty:
\examplebox{\bemph{\cmd{lawcite}\braces{casetest}} $\mapsto$\par\smallskip x\lawcite{casetest} y\lawcite{casetest}}

Illustration of @case fields: non-admiralty, AG/DPP is a party:
\examplebox{\bemph{\cmd{lawcite}\braces{casetestb}} $\mapsto$\par\smallskip x\lawcite{casetestb} y\lawcite{casetestb}}


Illustration of @case fields: non-admiralty, non-AG/DPP parties:
\examplebox{\bemph{\cmd{lawcite}\braces{casetestc}} $\mapsto$\par\smallskip x\lawcite{casetestc} y\lawcite{casetestc}}
...
For a statute, the field layout with the current lawcitestyle is shown in the following.

%\newpage
Illustration of @statute fields, with the citeref field set to \texttt{canleg}, using \cmd{lawcite}: 
\examplebox{\bemph{\cmd{lawcite}\braces{statutetest}} $\mapsto$\par\smallskip \lawcite{statutetest}}

Note that statutes appear inline by default, and are pre-formatted.

Illustration of @ljarticle fields, using \cmd{ljfootcite} (date field is numeric, and set to 2021): 
\examplebox{\bemph{\cmd{ljfootcite}\braces{ljarticletest}} $\mapsto$\par\smallskip x\ljfootcite{ljarticletest}}
...
%\begin{itemize}
%\item Installing the lawcite Biblatex files
%\item Biblatex option settings
%\item The \texttt{.bib} file
%			\begin{itemize}
%			\item bibentries
%			\item Bibliography management
%			\item Calling the \texttt{.bib} file(s)
%			\end{itemize}
%\item Numbered Paragraphs
%\item Citation Commands
%\item Bibliography Sections
%\item Table of Cases
%\item Index
%\end{itemize}
...
\dispeg{\cmdb{lawcite}\brackets{\cmd{protect}\cmd{label}\braces{fcoleb}}\brackets{}\braces{cole}}
...
\disp{\cmd{lawcite}\bracketsb{\textit{postnote}}\braces{key}}
\smallskip
...
\disp{\cmd{lawcite}\bracketsb{\textit{prenote}}\brackets{}\braces{key}}
\smallskip
\dispb{footer, single key}
...
\disp{\cmd{lawcite}\bracketsb{\textit{prenote}}\bracketsb{\textit{postnote}}\braces{key}}
\smallskip
\dispb{footer, single key, single cite}
...
\disp{\cmd{lawcitesfoot}\bracketsb{\textit{postnote}}\braces{key1,key2}}
\smallskip
...
\disp{\cmd{lawcitesfoot}\bracketsb{\textit{prenote}}\brackets{}\braces{key1,key2}}
\smallskip
\dispb{footer, multi key}
\dispeg{\cmdb{lawcitesfoot}\brackets{See also}\brackets{}\braces{mnr,randolph}}

Text\lawcitesfoot[See also][]{mnr,randolph}
\bigskip
\bigskip

%Dm
\disp{\textit{prenote} key1,key2 \textit{postnote}}\numeg
\disp{\cmd{lawcitesfoot}\parens{\textit{prenote}}\parens{\textit{postnote}}\braces{key1,key2}}
\smallskip
\dispb{footer, multi key, single cite}
\dispeg{\cmdb{lawcitesfoot}\parens{See also}\parens{both on point}\braces{mckay, opseu}}

Text\lawcitesfoot(See also)(both on point){mckay, opseu}
\bigskip
\bigskip
...
\dispeg{\cmdb{lawcitesfoot}\parens{There are two further cases\cmdb{addcomma}}\parens{\cmdb{addcomma}\cmdb{addspace} but neither case directly applies}\brackets{on prosecutorial immunity}\brackets{42}\braces{clark}\brackets{and speculation and the open court principle and sealing orders}\brackets{101}\braces{sherman}}

Text\lawcitesfoot(There are two further cases\addcomma)(\addcomma\addspace but neither case directly applies)[on prosecutorial immunity][42]{clark}[and speculation and the open court principle and sealing orders][101]{sherman}
\bigskip
\bigskip
...
ses droits possède une dimension de droit public''\footnote{\ljcite[\protect\label{cfn:otis}][at p 456]{otis}. \ljcite[See also][]{jukier}.}
...
\bigskip
Using \cmd{qqcite} and a bibentry.
...
For \lawcitetitle{cole}, see supra\sindex[general]{supra, manual}, n \ref{fcoleb}, and n \ref{fmcleanbclr} for \lawcitetitle{mcleanbclr}. These items use manual \cmd{label}ling via the prenote field when footnoting.
